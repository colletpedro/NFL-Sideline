...
public record EditorialBatchOptions(int season, LocalDate asOfDate, boolean dryRun,
                                    int maxAnalyses, String revisionKey, List<String> gameIds) {
    public EditorialBatchOptions {
        gameIds = List.copyOf(new LinkedHashSet<>(gameIds));
    }
    ...
    static EditorialBatchOptions parse(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            else if (argument.startsWith("--game-id=")) gameIds.add(required(argument.substring(10), "--game-id"));
            else if ("--game-id".equals(argument)) gameIds.add(value(args, ++i, argument));
        ...
        return new EditorialBatchOptions(season, asOf, dryRun, max, revision, List.copyOf(gameIds));
    }
    ...
    private static String value(String[] args, int index, String option) {
        ...
        return required(args[index], option);
    }

    private static String required(String value, String option) {
        if (value == null || value.isBlank()) throw new IllegalArgumentException(option + " requires a value");
        return value;
    }
}
